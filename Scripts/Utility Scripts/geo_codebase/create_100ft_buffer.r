...
retailers = read.csv("/Volumes/cnlab/GeoRemote/Data/Geodata/clean/100ft_buffer_data/unused_retailers.csv")

ppts = geodata$filename %>% unique()
files <- list.files("/Volumes/cnlab/GeoRemote/Data/Geodata/clean/100ft_buffer_data/", full.names = TRUE)
...
for (ppt_idx in ppts) {
  if (!sum(grepl(ppt_idx, files))) {
    ...
    exposureEvents = exposureEvents %>%
      bufferAndJoin( retailers, ., 2272, 100) %>% #set last number to the buffer zone you want in feet
    ...
    save(exposureEvents, file=sprintf("/Volumes/cnlab/GeoRemote/Data/Geodata/clean/100ft_buffer_data/%s_100ft_buffer.RData",ppt_idx))
    write.csv(retailers, file="/Volumes/cnlab/GeoRemote/Data/Geodata/clean/100ft_buffer_data/unused_retailers.csv")
  }
}
